...
data_root = 'data/LSA64/videos'
data_root_val = data_root
ann_file_train = f'data/LSA64/train_list.txt'
ann_file_val = f'data/LSA64/val_list.txt'
ann_file_test = f'data/LSA64/val_list.txt'
...
train_dataloader = dict(
    batch_size=16,       # 训练时每个单个 GPU 的批量大小
...
train_cfg = dict(                       # 训练循环的配置
    ...
    max_epochs=50,                      # 总的训练周期数
    val_begin=1,                        # 开始验证的训练周期
    dynamic_intervals=[(1, 5), (40, 1)])    # 验证间隔，从1开始每5轮验证一次，从20开始，一轮验证一次
...
optim_wrapper = dict(
    ...
    optimizer=dict(
        type='AdamW', lr=1e-3, betas=(0.9, 0.999), weight_decay=0.01),
